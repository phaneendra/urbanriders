...
Container sizes use the following `max-width` values, which may be [customized](/themes/docs/theme/token-reference#modifying-tokens) if needed.

<Table.Root variant="surface" my="5">
  <Table.Header>
    <Table.Row>
      <Table.ColumnHeaderCell>Size</Table.ColumnHeaderCell>
      <Table.ColumnHeaderCell>CSS token</Table.ColumnHeaderCell>
