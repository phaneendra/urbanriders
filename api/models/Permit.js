...
var Permit = {
    ...
    /**
     * Callback to be run before creating a Permit.
     *
     * @param {Object}   passport The soon-to-be-created Permit
     * @param {Function} next
     */
    beforeCreate: function(permit, next) {
        if (permit.hasOwnProperty('password')) {
            bcrypt.hash(permit.password, 10, function(err, hash) {
                permit.password = hash;
                next(err, permit);
            });
        } else {
            next(null, permit);
    ...
    /**
     * Callback to be run before updating a Permit.
     *
     * @param {Object}   permit Values to be updated
     * @param {Function} next
     */
    beforeUpdate: function(permit, next) {
        if (permit.hasOwnProperty('password')) {
            bcrypt.hash(permit.password, 10, function(err, hash) {
                permit.password = hash;
                next(err, permit);
