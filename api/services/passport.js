 ...
 *
 *   1. Given a provider and an identifier, find a mathcing permit.
 *   2. From here, the logic branches into two paths.
 *
 *     - A user is not currently logged in:
 *       1. If a permit wassn't found, create a new user as well as a new
 *          permit that will be assigned to the user.
 *       2. If a permit was found, get the user associated with the permit.
 *
 *     - A user is currently logged in:
 *       1. If a permit wasn't found, create a new permit and associate it
 *          with the already logged in user (ie. "Connect")
 *       2. If a permit was found, nothing needs to happen.
 *
...
passport.connect = function(req, query, profile, next) {
    ...
    // If the profile object contains a in, add it to the user.
    ...
    if (!Object.keys(user).length) {
        return next(new Error('Neither a username or email or profile id was available', null));
    }
    ...
    Permit.findOne({
        ...
        .populate('user')
        .exec(function(err, permit) {
            if (err) return next(err);

            if (!req.user) {
                // Scenario: A new user is attempting to sign up using a third-party
                //           authentication provider.
                // Action:   Create a new user and assign them a permit.
                if (!permit) {
                    User.create(user).exec(function(err, user) {
                        ...
                        Permit.create(query).exec(function(err, passport) {
                ...
                //           connected passport.
                // Action:   Get the user associated with the permit.
                else {
                    next(null, permit.user);
                }
            } else {
                // Scenario: A user is currently logged in and trying to connect with a new
                //           permit.
                // Action:   Create and assign a new permit to the user.
                if (!permit) {
                    query.user = req.user.id;

                    Permit.create(query).exec(function(err, permit) {
                        // If a permit wasn't created, bail out
                        if (err) return next(err);
...
passport.loadStrategies = function(req) {
    ...
    sails.log.debug("Load strategies strategies: " + JSON.stringify(strategies));
    ...
    Object.keys(strategies).forEach(function(key) {
        ...
        if (key === 'local') {
            ...
            var callback = path.join('auth', key, 'callback');
            ...
            sails.log.debug("Load strategies options: " + JSON.stringify(options));
